...
const SCREENS = [
  { name: 'splash', setup: (page) => page.waitFor(`!!document.getElementById('btn-splash-start')`, 20000) },
  {
    name: 'home',
    setup: async (page) => {
      await page.waitFor(`!!document.getElementById('btn-splash-start')`, 20000);
      await page.click('#btn-splash-start');
      await page.waitFor(`getComputedStyle(document.getElementById('home-screen')).display === 'flex'`, 20000);
      await waitHomeSettled(page);
    },
  },
  {
    name: 'arena',
    setup: (page) => page.waitFor(`getComputedStyle(document.getElementById('game-screen')).display === 'flex'`, 20000),
  },
  {
    name: 'archive-modal',
    setup: async (page) => {
      ...
      await page.waitFor(`getComputedStyle(document.getElementById('archive-modal')).display === 'flex'`, 20000);
    },
  },
  {
    name: 'rules-modal',
    setup: async (page) => {
      ...
      await page.waitFor(`getComputedStyle(document.getElementById('rules-modal')).display === 'flex'`, 20000);
    },
  },
];
...
async function main() {
  ...
  try {
    ...
    for (const screen of SCREENS) {
      ...
      if (verbose) console.error(`[a11y] ${screen.name}: ${violations.length} violation(s)`);
    }

    // end-overlay variants via hook
    for (const outcome of ['victory', 'defeat']) {
      await page.navigate(urlWith(SERVER, { test: 1, screen: 'arena', seed: 1, outcome }));
      await page.waitFor(`getComputedStyle(document.getElementById('end-overlay')).display === 'flex'`, 20000);
      const violations = await runAxe(page);
      report.screens.push({ name: `end-overlay-${outcome}`, violations });
    }

    // reward overlay via hook (between node clears)
    await page.navigate(urlWith(SERVER, { test: 1, screen: 'arena', seed: 1, outcome: 'reward' }));
    await page.waitFor(`getComputedStyle(document.getElementById('reward-overlay')).display === 'flex'`, 20000);
    const violations = await runAxe(page);
    report.screens.push({ name: 'reward-overlay', violations });

    const byImpact = (v) => ({ critical: 0, serious: 0, moderate: 0, minor: 0 }[v] || 0);
    ...
    for (const s of report.screens) {
      ...
      for (const v of s.violations) {
        ...
        grand[k] = (grand[k] || 0) + 1;
        seen.add(v.id);
        if ((k === 'critical' || k === 'serious') && !allCritical.find((x) => x.id === v.id)) {
          allCritical.push({ screen: s.name, id: v.id, impact: k, help: v.help, helpUrl: v.helpUrl, nodes: v.nodes.slice(0, 3) });
        }
      }
      const total = s.violations.length;
      console.log(`  ${total ? '⚠' : '✔'} ${s.name.padEnd(18)} ${total} violation(s)  [crit ${c.critical} · ser ${c.serious} · mod ${c.moderate} · min ${c.minor}]`);
    }
    console.log(`  totals: critical ${grand.critical} · serious ${grand.serious} · moderate ${grand.moderate} · minor ${grand.minor} (${seen.size} unique rule ids)`);
